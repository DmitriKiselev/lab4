...
def write(db_path, key_path, period):
    ...
    if ('data',) not in cursor.fetchall():
        cursor.execute("""CREATE TABLE data
                      (pressure_value, temprature1_value, temprature2_value,
                       co2_value, humidity_value, distance_value, fuel_value)
                    """)
    ...
    while 1:
        if time.time() - start == period:
            ...
            t1_sensor = f.encrypt(str(rescale(np.random.power(0.5), 0, 1,
                                              -40, 120)).encode("utf-8"))
            t2_sensor = f.encrypt(str(np.random.normal(20, 15)).encode("utf-8")
                                  )
            co2_sensor = f.encrypt(str(np.random.uniform(400, 2000)).encode
                                   ("utf-8"))
            humidity_sensor = np.random.laplace(loc=0.0, scale=1.0, size=None)
            humidity_sensor = f.encrypt(str(rescale(humidity_sensor, -5, 5, 10,
                                                    99)).encode("utf-8"))
            distance_sensor = np.random.power(0.7)
            distance_sensor = f.encrypt(str(rescale(distance_sensor, 0, 1, 0.1,
                                                    10)).encode("utf-8"))
...
def main():
    ...
    args = get_args()
    write(args.db_path, args.key_path, args.period)
